...
def RejectSingleImage(idx, msg=None):
...
def ClearImages():
	...
	ReducedImageArrays = []
	CentroidWindows = []
	CentroidDataFrame = pd.DataFrame()
...
if __name__=="__main__":
		...
		for win_num in range(len(CentroidWindows)):
			x0 = CentroidWindows[win_num][0][0]
			y0 = CentroidWindows[win_num][0][1]
			x1 = CentroidWindows[win_num][1][0]
			y1 = CentroidWindows[win_num][1][1]
			...
			for i in range(len(ReducedImageArrays)):
				...
				windowed_image_array = ReducedImageArrays[i][x0:x1,y0:y1]
